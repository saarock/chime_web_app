...
const ReportUser = ({ reportedUserId }: { reportedUserId: string }) => {
  // Fall  back errror handler
  const { setErrorMessageFallBack } = useErrorHandlerAtPageAndComponentLevel();
  // Helps to track the loading state and success state
  ...
  const handleClick = useCallback(
    async (type: "like" | "dislike") => {
      ...
      try {
        ...
        if (!reportedUserId) {
          toast.error(
            "There is some-thing bug. Pleased while starting another call reload the app and start the random-call"
          );
          return;
        }
        if (!type) {
