...
vi.mock('@react-oauth/google', () => ({
    GoogleLogin: ({ onSuccess }: any) => (
        <button onClick={() => onSuccess({ credential: 'test-credential', clientId: 'test-client-id' })}>Google Login Button</button>
    )
}));
...
vi.mock('../../apps', async (importOriginal) => {
    ...
    return {
        ...
        serverLoginWithGoogle: vi.fn(() => () => Promise.resolve({ unwrap: () => Promise.resolve() }))
    };
});
