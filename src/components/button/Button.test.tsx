...
describe("Button component", () => {
    ...
    it("applies secondary ", () => {
        render(<Button text="Secondary" onClick={() => { }} variant={Variant.secondary} />);
        const button = screen.getByRole("button", { name: "Secondary" });
        expect(button).toHaveClass("chime-btn-secondary")
    });
    it("applies secondary ", () => {
        render(<Button text="Ternary" onClick={() => { }} variant={Variant.ternary} />);
        const button = screen.getByRole("button", { name: "Ternary" });
        expect(button).toHaveClass("chime-btn-ternary")
    });

    it("applies secondary ", () => {
        render(<Button type="submit" text="Submit" onClick={() => { }} variant={Variant.secondary} />);
